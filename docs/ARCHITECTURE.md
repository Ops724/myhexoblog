...
| `source/_data/profile.yml` | 私有 | 站点名、副标题、头像、分享图（`og_image`）、图标、背景色、备案号（`icp`、`police_icp`、`police_icp_icon`） |
...
- **索引**：`/search-index.json`（中文）与 `/en/search-index.json`（英文），由 `generators/localized-search-index.js` 生成；收录文章、相册，以及**有正文的独立页面**（如关于页，没有正文的分类总览与搜索页会被过滤掉）；每条包含标题、地址、频道、日期与正文纯文本（相册用照片说明兜底），单条正文最多 6000 字符。
