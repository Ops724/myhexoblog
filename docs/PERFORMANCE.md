...
| 文件 | 变化 | 说明 |
| --- | --- | --- |
| `source/images/avatar.png` | 9.9 KB → **24.8 KB** | 换成新 logo；页头显示尺寸放大到 64×64，所以素材做成 128×128 |
...
- 头像从原图**裁到头部**再缩放：页头显示尺寸只有 64×64，整幅插画缩下去只剩一个模糊人影，裁到头部才认得出是张脸；分享图反过来用完整原图，缩放后左右补白到 1200×630，不裁切。
- 头像素材的像素尺寸取「显示尺寸的 2 倍」（64×64 显示 → 128×128 素材），视网膜屏下才不糊；这也是上一版 48×48 显示配 96×96 素材的同一条规则。
