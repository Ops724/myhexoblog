...
以下事项已经定下来，但**不进入开发流程**，由使用者手动执行：

- **图片压缩**：文章截图（PNG 约 12.8MB）与头像（PNG 524KB）体积偏大，需要手动压缩或转格式；建议先备份原图。体积基线、处理建议与验证方式见 [docs/PERFORMANCE.md](./docs/PERFORMANCE.md)。
- **站点图标（.ico）**：保持 `.ico` 格式，由使用者用在线生成器或本地工具重新导出（只需要 16/32/48 三个尺寸）；本机没有 ImageMagick、Pillow 等工具，`sips` 又不支持写 `.ico`。完整步骤见 [docs/PERFORMANCE.md](./docs/PERFORMANCE.md) 第四节。
