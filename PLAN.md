...
- 设计思路（关键决策都写在这里）：
  1. **为什么选 Pagefind**：它在构建产物上建立静态索引，运行时纯前端按需加载，不需要后端服务，和「ECS 只放静态文件 + rsync 发布」完全契合；相比 searchdb 这类插件方案，中文分词与高亮体验更好；相比 Algolia 等第三方服务，不需要密钥与外部依赖。
  2. **构建流程变化**：索引必须在 Hexo 生成之后建立。
     - `package.json` 增加 `search:index`（`pagefind --site public`）与 `build:full`（生成站点后接着建索引）
     - `tools/deploy.sh` 的构建步骤改为走 `build:full`，否则线上搜索页没有索引
     - `pagefind` 作为 devDependency 引入，这是里程碑 2 唯一新增的依赖，换来的是零后端搜索
     - 本地 `hexo server` 不会自动建索引：搜索页在本地要先跑一次 `npm run search:index`，这一点写进 README
  3. **索引范围**：只索引文章正文——正文容器加 `data-pagefind-body`，页头、页脚、分页等加 `data-pagefind-ignore`，避免导航文字污染结果。
  4. **双语隔离**：采用**单一索引 + 语言过滤器**，不建两份索引。模板在外层容器输出 `data-pagefind-filter="language:zh-CN"`（英文页为 `en`），搜索页按当前语言把过滤器传给 Pagefind。好处是构建只跑一次、资源路径简单；代价是索引里同时存在两种语言的数据。等英文内容规模变大再升级为两份索引（`--output-subdir`）。
  5. **搜索页**：新增 `/search/` 与 `/en/search/`（`source/search/index.md`、`source/en/search/index.md`，布局 `search`），主题新增 `layout/search.ejs`：输入框、结果列表与空状态；Pagefind 的 JS 与 CSS 只在搜索页加载，其它页面零开销；界面文案走 i18n。
  6. **降级**：关闭 JavaScript 时给出提示，不影响其它页面。
  7. **导航**：`navigation.yml` 增加搜索入口。
- 任务：安装依赖、构建脚本与部署脚本、搜索页与布局、索引标记、语言过滤、导航、文档
- 验收：`npm run build:full` 后生成 `public/pagefind/`；`/search/` 能搜到中文文章（例如搜 `Go` 命中三篇）；`/en/search/` 在英文为空时返回空结果且不会串出中文；结果高亮正常；其它页面不加载 Pagefind 资源。
...
| Pagefind 构建步骤 | 多一步建索引，本地开发需要手动执行 | README 写清楚；部署脚本统一走 `build:full` |
| 搜索语言隔离 | 采用单索引加过滤器，索引同时含两种语言 | 英文内容变多后评估改为两份索引 |
